...
void loop() {
  if (Serial.available() > 0) {
    ...
    // Update cmd only if it's a valid command
    if (incoming == 'r' || incoming == 'f' || incoming == 't' || incoming == 'g' || 
        incoming == 'y' || incoming == 'h' || incoming == 'u' || incoming == 'j' || 
        incoming == 'w' || incoming == 's' || incoming == 'a' || incoming == 'd' || 
        incoming == 'x') {
      cmd = incoming;
    
      // Set all PWM outputs to zero initially
      ledcWrite(pwm_la1, 0);
      ledcWrite(pwm_la2, 0);
      ledcWrite(pwm_la3, 0);
      ledcWrite(pwm_w1, 0);
